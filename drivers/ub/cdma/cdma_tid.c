...
static int cdma_enable_sva_feature(struct cdma_dev *cdev)
{
	...
	ret = ummu_dev_enable_feat(cdev->dev, IOMMU_DEV_FEAT_IOPF);
	if (ret) {
...
disable_iopf_feature:
	ummu_dev_disable_feat(cdev->dev, IOMMU_DEV_FEAT_IOPF);
disable_ksva_feature:
	ummu_dev_disable_feat(cdev->dev, IOMMU_DEV_FEAT_KSVA);
...
{
	ummu_dev_disable_feat(cdev->dev, IOMMU_DEV_FEAT_SVA);
	ummu_dev_disable_feat(cdev->dev, IOMMU_DEV_FEAT_IOPF);
	ummu_dev_disable_feat(cdev->dev, IOMMU_DEV_FEAT_KSVA);
}
