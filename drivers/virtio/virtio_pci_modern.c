...
static int vp_modern_get_queue_reset(struct virtio_pci_modern_device *mdev, u16 index)
{
...
static void vp_modern_set_queue_reset(struct virtio_pci_modern_device *mdev, u16 index)
{
	struct virtio_pci_modern_common_cfg __iomem *cfg;
...
static int vp_active_vq(struct virtqueue *vq, u16 msix_vec)
{
	...
	if (msix_vec != VIRTIO_MSI_NO_VECTOR) {
		msix_vec = vp_modern_queue_vector(&vp_dev->mdev, index, msix_vec);
		if (msix_vec == VIRTIO_MSI_NO_VECTOR)
			return -EBUSY;
	}
...
static int vp_modern_disable_vq_and_reset(struct virtqueue *vq)
{
	...
	vp_modern_set_queue_reset(&vp_dev->mdev, vq->index);
...
static int vp_modern_enable_vq_after_reset(struct virtqueue *vq)
{
	...
	if (vp_modern_get_queue_reset(&vp_dev->mdev, index))
		return -EBUSY;

	if (vp_modern_get_queue_enable(&vp_dev->mdev, index))
		return -EBUSY;
