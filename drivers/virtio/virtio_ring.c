					  ...
					  struct vring_desc_extra *extra)
{
	...
	if (flags & VRING_DESC_F_INDIRECT) {
		if (!vq->use_map_api || vq->save_indir)
			goto out;
	} else if (vq->save_indir || !vring_need_unmap_buffer(vq, extra))
		goto out;
...
static inline int virtqueue_add_split_rpair(struct vring_virtqueue *vq,
				      struct scatterlist *sgs[],
				      unsigned int total_sg,
				      unsigned int out_sgs,
				      unsigned int in_sgs,
				      ...
				      unsigned long attr)
{
	...
	for (n = 0; n < out_sgs; n++) {
		...
		if ((n == 0) && indirect && vq->save_indir) {
			prev = i;
			i = virtqueue_add_desc_split(vq, desc, extra, i, l1_addr,
						total_sg * sizeof(struct vring_desc),
				...
				gfp_t gfp)
{
	...
	return virtqueue_add_split_rpair(vq, sgs, total_sg,
				   out_sgs, in_sgs, data, ctx, premapped, gfp, 0);
}
