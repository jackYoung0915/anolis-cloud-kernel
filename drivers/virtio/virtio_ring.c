				      ...
				      gfp_t gfp)
{
	...
	for (n = 0; n < out_sgs; n++) {
		...
		if ((n == 0) && indirect && vq->save_indir) {
			prev = i;
			i = virtqueue_add_desc_split(_vq, desc, i, l1_addr,
							total_sg * sizeof(struct vring_desc),
							VRING_DESC_F_NEXT,
							indirect);
		}
					...
					bool callback)
{
	...
	vring_packed->used_wrap_counter = 1;
		      ...
		      gfp_t gfp)
{
	...
	if (!vq->save_indir) {
		pr_err("virtqueue_add_rpair() only supported for SQ.");
		return -EINVAL;
	}
...
int virtqueue_set_dma_premapped(struct virtqueue *_vq)
{
	...
	if (num != vq->vq.num_free) {
		END_USE(vq);
		pr_debug("%s:%d err num is not equal to num_free, num=%u, num_free=%u\n",
					__func__, __LINE__, num, vq->vq.num_free);
		return -EINVAL;
	}

	if (!vq->use_dma_api) {
		END_USE(vq);
		pr_debug("%s:%d err vring does not use the dma api\n",
					__func__, __LINE__);
		return -EINVAL;
	}
