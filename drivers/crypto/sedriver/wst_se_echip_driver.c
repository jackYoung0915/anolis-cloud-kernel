...
static void SE_RESET(struct tagSEdrvctl *pdrvctl)
{
	...
	mdelay(300);
	ulreg64 = readq((void __iomem *)uladdr);
	if ((ulreg64 & 0xf0000000000000) != 0xf0000000000000)
		writeq(ulreg64 | 0xf0000000000000, (void __iomem *)uladdr);
...
PROG:
		...
		psendpackage =
			(struct ST_SEND_PACKAGE *)wst_Popfront_Que(&g_SendQueueContainer);
		if (!psendpackage) {
			...
			while (time_before(jiffies, ultimeout)) {
				if (pdrvctl->ulCurrBdReadPtr[iChannel] ==
					...
					if (down_timeout(&(pdrvctl->sema), 1 * HZ))
						pr_info("donw_timeout don't success!\n");
...
static irqreturn_t wst_low_channel_status(int irq, void *p)
{
	struct tagSEdrvctl *pdrvctl = (struct tagSEdrvctl *)p;
	int64_t ulIntStat = 0;
	unsigned long ulflag;
	...
	for (i = 0; i <= 1; i++) {
		...
		while (1) {
			if (ulCurrReadPtr != ulReadPtr) {
				pCurBD = &((struct tagSEBasicBD *)(pdrvctl->ulBDMemBase
								   [iChannel]))
						 [ulCurrReadPtr];
					...
					if ((pdrvctl->ikernel)[iChannel]
						...
						if (pdrvctl->pcallback
							...
							struct ST_INT_MESSAGE *intmessage =
								NULL;
							unsigned long ulflags1;
							intmessage = (struct ST_INT_MESSAGE *)kmalloc(
								sizeof(struct ST_INT_MESSAGE),
								GFP_ATOMIC);
							if (!intmessage)
								return IRQ_HANDLED;
