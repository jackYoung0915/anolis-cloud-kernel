...
#define HandleRead32(handle, addr, pData)                             \
	do {                                                          \
		/*Ensure write global visibility and prevent reordering of subsequent stores*/	\
		smp_mb();                                             \
...
#define HandleRead64(handle, addr, pData)                             \
	do {                                                          \
		/*Full memory barrier guarantees 8-byte load atomicity*/	\
		smp_mb();                                             \
...
#define HandleWrite64(handle, addr, value)                         \
	do {                                                       \
		...
		/*Ensure double-word write completion and maintain cache coherency protocols*/	\
		smp_mb();                                          \
	} while (0)
...
#ifdef DEBUG_DRIVER
#define TRACEMSG(fmt, args...) pr_debug("msg: " fmt, ##args)
#else
#define TRACEMSG(fmt, args...)
#endif

#ifdef DEBUG_DRIVER_ERROR
#define TRACEERR(fmt, args...) pr_debug("err: " fmt, ##args)
#else
