...
static int get_queue_head(int prio)
{
	struct csv_ringbuffer_queue *ringbuffer = &vpsp_ring_buffer[prio];
...
static void vpsp_ring_update_head(int prio, uint32_t new_head)
{
	...
	uint32_t orig_head = get_queue_head(prio);
...
static int __vpsp_do_ringbuf_cmds_locked(void)
{
	...
	rb_head |= (get_queue_head(CSV_COMMAND_PRIORITY_HIGH) << PSP_RBHEAD_QHI_HEAD_SHIFT);
	rb_head &= (~PSP_RBHEAD_QLO_HEAD_MASK);
	rb_head |= get_queue_head(CSV_COMMAND_PRIORITY_LOW);
...
int vpsp_try_get_result(struct vpsp_cmd_ctx *cmd_ctx, struct vpsp_ret *psp_ret)
{
	...
	if (vpsp_psp_mutex_trylock()) {
		/*
		 * Use mailbox mode to execute a command if there is
		 * only one command
		 */
		...
		/* Change the command to the running state if getting the mutex
		 * fails
		 */
		psp_ret->status = VPSP_RUNNING;
		return 0;
