...
static void cppc_scale_freq_workfn(struct kthread_work *work)
{
	...
	if (ret) {
		...
		topology_clear_scale_freq_source(SCALE_FREQ_SOURCE_CPPC, cpu_data->shared_cpu_map);
		return;
	}
...
static int cppc_cpufreq_cpu_init(struct cpufreq_policy *policy)
{
	...
	policy->max = cppc_cpufreq_perf_to_khz(cpu_data, policy->boost_enabled ? cpu_data->perf_caps.highest_perf : cpu_data->perf_caps.nominal_perf);
...
static unsigned int cppc_cpufreq_get_rate(unsigned int cpu)
{
	...
	if (ret)
		return 0;
	delivered_perf = cppc_perf_from_fbctrs(cpu_data, &fb_ctrs.fb_ctrs_t0, &fb_ctrs.fb_ctrs_t1);
