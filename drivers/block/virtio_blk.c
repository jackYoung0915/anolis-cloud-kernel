				...
				struct request *req, struct virtblk_req *vbr)
{
	...
	return blk_rq_map_sg_bidir(req, vbr->sg_table.sgl,
				vbr->sg_table_extra.sgl);
}
...
static void virtio_queue_rqs_rpair(struct rq_list *rqlist)
{
...
static int check_ext_feature(struct virtio_blk *vblk, void __iomem *ioaddr,
...
static int init_vq_rpair(struct virtio_blk *vblk)
{
	...
	for (i = 0; i < num_vqs - num_poll_vqs; i++) {
		...
		if (role == VIRTBLK_RING_SQ) {
			vqs_info[i].callback = NULL;
			snprintf(vblk->vqs[i].name, VQ_NAME_LEN, "req.%u", index);
		} else {
			vqs_info[i].callback = virtblk_done_rpair;
...
#ifdef CONFIG_VIRTIO_BLK_RING_PAIR
static void blk_mq_virtio_map_queues_rpair(struct blk_mq_queue_map *qmap,
...
static int virtblk_map_user_request(struct request *req, uintptr_t ubuffer,
		...
		unsigned int issue_flags, bool vec, unsigned int num)
{
		...
		struct io_uring_cmd *ioucmd, unsigned int issue_flags, bool vec)
{
	...
	if (data && data_len) {
		ret = virtblk_map_user_request(req, data, data_len, ioucmd, issue_flags,
					       vec, write_iov_count);
		if (ret)
			return ret;
	} else {
...
module_param_named(dyn_max_rqs, virtblk_dyn_max_rqs, ushort, 0444);
...
static int virtblk_probe(struct virtio_device *vdev)
{
	...
	if (vblk->ring_pair) {
		 ...
		 */
		vblk->tag_set.flags = BLK_MQ_F_DYN_ALLOC | BLK_MQ_F_NO_SCHED_BY_DEFAULT;
