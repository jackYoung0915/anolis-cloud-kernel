...
static int ublk_dev_param_zoned_apply(struct ublk_device *ub)
{
	...
	blk_queue_set_zoned(ub->ub_disk, BLK_ZONED_HM);
	blk_queue_required_elevator_features(ub->ub_disk->queue,
					     ELEVATOR_F_ZBD_SEQ_WRITE);
	blk_queue_max_active_zones(ub->ub_disk->queue, p->max_active_zones);
	blk_queue_max_open_zones(ub->ub_disk->queue, p->max_open_zones);
	blk_queue_max_zone_append_sectors(ub->ub_disk->queue, p->max_zone_append_sectors);

	ub->ub_disk->queue->nr_zones = ublk_get_nr_zones(ub);
		      ...
		      unsigned int nr_zones, report_zones_cb cb, void *data)
{
	...
	nr_zones = min_t(unsigned int, ub->ub_disk->queue->nr_zones - first_zone,
			 nr_zones);
	...
	while (done_zones < nr_zones) {
		...
		status = blk_execute_rq(disk, req, 0);
		ret = blk_status_to_errno(status);
		blk_mq_free_request(req);
		if (ret)
			goto out;

		for (i = 0; i < zones_in_request; i++) {
		...
		size_t total, size_t pg_off, int dir)
{
	...
	while (done < total) {
		...
		void *bv_buf = kmap(bv.bv_page) + bv.bv_offset;
		void *pg_buf = kmap(data->pages[pg_idx]);
		...
		/* advance bio */
		bio_advance_iter(data->bio, &data->iter, bytes);
		...
		unsigned offset, struct iov_iter *uiter, int dir)
{
	...
	while (iov_iter_count(uiter) && iter.bio) {
		...
		len = iov_iter_get_pages(uiter, iter.pages,
				iov_iter_count(uiter),
				UBLK_MAX_PIN_PAGES, &off);
		if (len >= 0)
			iov_iter_advance(uiter, len);
		if (len <= 0)
			return done;
...
static enum blk_eh_timer_return ublk_timeout(struct request *rq, bool reserved)
{
...
static void ublk_cmd_cancel_cb(struct io_uring_cmd *cmd)
{
...
static ssize_t ublk_ch_read_iter(struct kiocb *iocb, struct iov_iter *to)
{
	...
	req = ublk_check_and_get_req(iocb, to, &buf_off, READ);
	if (IS_ERR(req))
		return PTR_ERR(req);

	ret = ublk_copy_user_pages(req, buf_off, to, READ);
...
static ssize_t ublk_ch_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	...
	req = ublk_check_and_get_req(iocb, from, &buf_off, WRITE);
	if (IS_ERR(req))
		return PTR_ERR(req);

	ret = ublk_copy_user_pages(req, buf_off, from, WRITE);
	ubq = req->mq_hctx->driver_data;
	ublk_put_req_ref(ubq, req);
...
{
	const struct ublksrv_ctrl_cmd *header = (struct ublksrv_ctrl_cmd *)cmd->cmd;
	void __user *argp = (void __user *)(unsigned long)header->addr;
		...
		const char *dev_path, int mask)
{
	...
	err = inode_permission(d_backing_inode(path.dentry), mask);
exit:
	path_put(&path);
	return err;
}
