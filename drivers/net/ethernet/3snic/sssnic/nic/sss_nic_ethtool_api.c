				     ...
				     const struct ethtool_coalesce *coal)
{
	...
	if (memcmp(coal, &tmp_coal, sizeof(tmp_coal)) != 0) {
		nicif_err(nic_dev, drv, netdev,
			  "Invalid parameter, only support to change rx/tx-usecs and rx/tx-frames\n");
		return -EOPNOTSUPP;
	}
...
static struct sk_buff *sss_nic_alloc_loop_skb(struct sss_nic_dev *nic_dev)
{
	...
	eth_hdr = (struct ethhdr *)__skb_put(skb, ETH_HLEN);
