...
int sss_nic_get_settings(struct net_device *netdev, struct ethtool_cmd *ep)
{
	struct sss_nic_cmd_link_settings settings = { { 0 } };
	int err;

	err = sss_nic_get_link_setting(netdev, &settings);
	if (err)
		return err;
...
int sss_nic_set_settings(struct net_device *netdev,
			struct ethtool_cmd *link_settings)
{
	/* Only support to set autoneg and speed */
	return sssnic_set_link_settings(netdev, link_settings->autoneg,
