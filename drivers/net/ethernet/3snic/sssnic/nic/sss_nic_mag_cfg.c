...
int sss_nic_get_sfp_eeprom(struct sss_nic_dev *nic_dev, u8 *data, u32 len)
{
	...
	mutex_lock(&nic_dev->sfp_mutex);
	if (jiffies_to_msecs(jiffies - nic_dev->last_jitters) < 1000) {
		memcpy(data, nic_dev->last_resp, sizeof(nic_dev->last_resp));
		mutex_unlock(&nic_dev->sfp_mutex);
		nic_info(nic_dev->dev_hdl, "use cached sfp eeprom");
		return 0;
	}
	mutex_unlock(&nic_dev->sfp_mutex);
	...
	mutex_lock(&nic_dev->sfp_mutex);
	memcpy(nic_dev->last_resp, xsfp_info.sfp_info, sizeof(nic_dev->last_resp));
	nic_dev->last_jitters = jiffies;
	mutex_unlock(&nic_dev->sfp_mutex);
	nic_info(nic_dev->dev_hdl, "update sfp eeprom");
