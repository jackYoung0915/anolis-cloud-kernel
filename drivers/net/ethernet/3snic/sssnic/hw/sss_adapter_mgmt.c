...
static unsigned char sss_get_pci_bus_id(struct sss_pci_adapter *adapter, int *domain)
{
	...
	*domain = 0;
	...
	if (!pci_is_root_bus(adapter->pcidev->bus)) {
		...
		*domain = pci_domain_nr(adapter->pcidev->bus);
	}
...
int sss_alloc_chip_node(struct sss_pci_adapter *adapter)
{
	...
	bus_id = sss_get_pci_bus_id(adapter, &domain);

	if (sss_chip_node_exist(adapter, bus_id, domain))
		return 0;
