...
#define SUMMARY "Chengdu NorthLink Ethernet Connection N5/N6 Series Linux Driver"
...
static const struct pci_device_id ne6x_pci_tbl[] = {
	{PCI_VDEVICE(NORTHLINK, 0x5010), 0},
	{PCI_VDEVICE(NORTHLINK, 0x5011), 0},
	{PCI_VDEVICE(NORTHLINK, 0x6010), 0},
	{PCI_VDEVICE(NORTHLINK, 0x6011), 0},
	/* required last entry */
	{0, 0},
};
...
MODULE_DESCRIPTION("Chengdu NorthLink Ethernet Connection N5/N6 Series Linux Driver");
MODULE_LICENSE("GPL");
...
static void ne6x_link_event(struct ne6x_pf *pf)
{
	...
	for (i = 0; i < pf->num_alloc_adpt; i++) {
			...
			if ((phy_info->link_info.ext_info & MEDIA_ONLINE_BIT) !=
				...
				if (!(phy_info->link_info.ext_info & MEDIA_ONLINE_BIT)) {
					netdev_info(adpt->netdev, "adpt->id= %d,optical module unplugged\n",
						    adpt->idx);
					adpt->port_info->phy.media_type	= NE6X_MEDIA_UNKNOWN;
				} else {
		...
		if (phy_info->link_info.ext_info & MEDIA_ONLINE_BIT) {
			if (phy_info->link_info.ext_info & MEDIA_DAC_BIT)
				adpt->port_info->phy.media_type = NE6X_MEDIA_DA;
			else
				adpt->port_info->phy.media_type = NE6X_MEDIA_FIBER;
		}
...
static const struct net_device_ops ne6x_netdev_ops = {
	...
	.ndo_set_vf_spoofchk = ne6x_set_vf_spoofchk,
...
static int ne6x_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	clear_bit(NE6X_PF_NIC_RESET, pf->state);
