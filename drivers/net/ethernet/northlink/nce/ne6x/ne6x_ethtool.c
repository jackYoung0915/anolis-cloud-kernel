				   ...
				   struct ethtool_link_ksettings *ks)
{
	...
	if (adpt->port_info->phy.media_type == NE6X_MEDIA_DA) {
		if (ne6x_dev_check_media(adpt, PORT_DA))
			netdev_info(adpt->netdev, "DAC not support\n");
				   ...
				   const struct ethtool_link_ksettings *ks)
{
	...
	if (ne6x_dev_check_speed_change(adpt)) {
...
static void ne6x_get_drvinfo(struct net_device *netdev, struct ethtool_drvinfo *drvinfo)
{
	...
	soc_ver = pf->fw_info.verinfo.firmware_soc_ver;
	np_ver = pf->fw_info.verinfo.firmware_np_ver & 0xFFFF;
	erom_ver = pf->fw_info.verinfo.firmware_pxe_ver & 0xFFFF;
...
static void ne6x_cvt_fec(enum ne6x_fec_state *fec)
{
	switch (*fec) {
	case NE6X_FEC_NONE:
		*fec = ETHTOOL_FEC_OFF;
...
static int ne6x_set_fec_param(struct net_device *netdev, struct ethtool_fecparam *fecparam)
{
	...
	if (ne6x_dev_check_fec_capability(adpt, fec))
		return -EOPNOTSUPP;
...
static int ne6x_reset(struct net_device *netdev, u32 *flags)
{
	...
	set_bit(NE6X_PF_NIC_RESET, adpt->back->state);
