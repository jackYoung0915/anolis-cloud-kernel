...
/* Copyright (c) 2020 - 2023, Chengdu NorthLink Technology Co., Ltd. */
...
#define SUMMARY \
	"Chengdu NorthLink Ethernet Connection N5/N6 Series Virtual Function Linux Driver"
#define COPYRIGHT "Copyright (c)  2020 - 2023 Chengdu NorthLink Technology Co., Ltd."
...
static const struct pci_device_id ne6xvf_pci_tbl[] = {
	{PCI_VDEVICE(NORTHLINK, 0x501a), 0},
	{PCI_VDEVICE(NORTHLINK, 0x601a), 0},
	/* required last entry */
	{0,}
};
...
MODULE_AUTHOR("Chengdu NorthLink Technology Co., Ltd., <support@bzwx-kj.com>");
...
static int ne6xvf_init_get_resources(struct ne6xvf_adapter *adapter)
{
	...
	if (!adapter->vf_res) {
		adapter->vf_res = kzalloc(struct_size(adapter->vf_res, vsi_res, 1), GFP_KERNEL);
		if (!adapter->vf_res)
			goto err;
	}
...
static void ne6xvf_disable_vf(struct ne6xvf_adapter *adapter)
{
	...
	ne6xvf_free_queues(adapter);
	memset(adapter->vf_res, 0, struct_size(adapter->vf_res, vsi_res, 1));
...
static int ne6xvf_request_traffic_irqs(struct ne6xvf_adapter *adapter, char *basename)
{
	...
	for (vector = 0; vector < q_vectors; vector++) {
		 ...
		 */
		cpu = cpumask_local_spread(q_vector->v_idx, NUMA_NO_NODE);
...
static void ne6xvf_configure_queues(struct ne6xvf_adapter *adapter)
{
	...
	for (i = 0; i < adapter->num_active_queues; i++) {
		...
		cq_cfg.reg.csr_cq_merge_time_vp = 2;
 ...
 */
static int ne6xvf_init_process_extended_caps(struct ne6xvf_adapter *adapter)
{
	struct pci_dev *pdev = adapter->pdev;
	int ret;
	...
	ret = ne6xvf_request_feature(adapter);
	if (ret) {
		dev_err(&pdev->dev, "Unable to request feature (%d)\n", ret);
		return ret;
	}

	ret = ne6xvf_get_vf_feature(adapter);
	if (ret) {
		dev_err(&pdev->dev, "Unable to get vf feature (%d)\n", ret);
		return ret;
	}

	/* read the initial spoofchk status from hardware */
	adapter->hw_feature &= NE6X_F_TX_SPOOF_CHECK;

	/* Clear promis when init */
...
static int ne6xvf_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	adapter->aq_required = 0;
	if (ne6xvf_init_process_extended_caps(adapter)) {
		err = -EIO;
		goto err_caps;
	}
...
static inline void ne6xvf_shutdown(struct pci_dev *pdev)
{
	struct ne6xvf_adapter *adapter = ne6xvf_pdev_to_adapter(pdev);
