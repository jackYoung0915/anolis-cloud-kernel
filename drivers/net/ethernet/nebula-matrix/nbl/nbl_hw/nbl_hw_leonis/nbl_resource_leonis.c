...
static int nbl_res_get_queue_num(void *priv, u16 func_id, u16 *tx_queue_num, u16 *rx_queue_num);
				...
				struct nbl_register_net_result *register_result)
{
	...
	if (common->is_vf) {
		tx_queue_num = NBL_LOONGSON64_VF_MAX_QUEUE_NUM;
		rx_queue_num = NBL_LOONGSON64_VF_MAX_QUEUE_NUM;
	} else {
		tx_queue_num = NBL_LOONGSON64_MAX_QUEUE_NUM;
		rx_queue_num = NBL_LOONGSON64_MAX_QUEUE_NUM;
	}
#else
...
static int nbl_res_get_ustore_pkt_drop_stats(void *priv)
{
	...
	for (i = 0; i < eth_info->eth_num; i++) {
		...
		ustore_stats[eth_id].rx_drop_pkt += ustore_stats_temp.rx_drop_pkt;
		ustore_stats[eth_id].rx_truncate_pkt += ustore_stats_temp.rx_truncate_pkt;
	}
						   ...
						   struct nbl_ustore_stats *nbl_ustore_stats)
{
	...
	nbl_ustore_stats->rx_drop_pkt = ustore_stats[eth_id].rx_drop_pkt;
	nbl_ustore_stats->rx_truncate_pkt = ustore_stats[eth_id].rx_truncate_pkt;
	return 0;
}
