...
static u16 nbl_flow_cfg_action_set_dport_mcc_lag(u16 lag_id)
{
	union nbl_action_data set_dport = {.data = 0};
	...
	set_dport.dport.down.eth_vld = 0;
	set_dport.dport.down.lag_id = lag_id;
			     ...
			     s32 type, struct nbl_flow_fem_entry *flow)
{
	...
	tcam_item = kzalloc(sizeof(*tcam_item), GFP_KERNEL);
	if (!tcam_item)
		return -ENOMEM;
	...
	if (type >= NBL_FLOW_TYPE_MAX && type < NBL_FLOW_ACCEL_MAX) {
...
static void nbl_flow_del_flow(struct nbl_resource_mgt *res_mgt, struct nbl_flow_fem_entry *flow)
{
	...
	if (flow->type >= NBL_FLOW_TYPE_MAX && flow->type < NBL_FLOW_ACCEL_MAX) {
		if (flow->flow_type == NBL_KT_FULL_MODE)
...
static int nbl_flow_add_mcc_node(struct nbl_flow_multi_group *multi_group,
				 ...
				 int type, u16 data, u16 mcc_id, u16 head)
{
	struct nbl_flow_mcc_node *mcc_node = NULL, *mcc_head = NULL;
	struct nbl_phy_ops *phy_ops;
	...
	mcc_node->mcc_head = head;

	/* mcc_head must init before mcc_list */
	if (head) {
		list_add_tail(&mcc_node->node, &multi_group->mcc_head);
		prev_mcc_id = NBL_MCC_ID_INVALID;

		WARN_ON(!nbl_list_empty(&multi_group->mcc_list));
		ret = phy_ops->add_mcc(NBL_RES_MGT_TO_PHY_PRIV(res_mgt), mcc_id,
	...
	/* first add mcc_list */
	if (prev_mcc_id == NBL_MCC_ID_INVALID && !nbl_list_empty(&multi_group->mcc_head)) {
		list_for_each_entry(mcc_head, &multi_group->mcc_head, node) {
				  ...
				  struct nbl_flow_mcc_node *mcc_node)
{
	...
	if (nbl_list_entry_is_head(mcc_node, &multi_group->mcc_list, node) ||
	    nbl_list_entry_is_head(mcc_node, &multi_group->mcc_head, node))
		return;
...
static void nbl_flow_del_lag(void *priv, u16 vsi)
{
	...
	if (nbl_list_entry_is_head(rule, &flow_mgt->lacp_list, node))
		return;
...
static void nbl_flow_del_lldp(void *priv, u16 vsi)
{
	...
	if (nbl_list_entry_is_head(rule, &flow_mgt->lldp_list, node))
		return;
...
static int nbl_flow_cfg_lag_mcc(void *priv, u16 eth_id, u16 lag_id, bool enable)
{
	struct nbl_resource_mgt *res_mgt = (struct nbl_resource_mgt *)priv;
	struct nbl_flow_mgt *flow_mgt = NBL_RES_MGT_TO_FLOW_MGT(res_mgt);
	...
	u16 mcc_id, mcc_action;

	multi_group = &flow_mgt->multi_flow[eth_id];
	NBL_FLOW_MCC_INDEX_KEY_INIT(&index_key, NBL_MCC_INDEX_ETH, eth_id);
	mcc_id = nbl_common_get_index(flow_mgt->mcc_tbl_priv, &index_key, NULL);

	mcc_action = enable ? nbl_flow_cfg_action_set_dport_mcc_lag(lag_id)
			    : nbl_flow_cfg_action_set_dport_mcc_eth(eth_id);

	return phy_ops->cfg_lag_mcc(NBL_RES_MGT_TO_PHY_PRIV(res_mgt), mcc_id, mcc_action);
}
...
static int nbl_flow_add_multi_rule(void *priv, u16 vsi)
{
	...
	return nbl_flow_add_mcc_node(multi_group, res_mgt, NBL_MCC_INDEX_VSI, vsi, mcc_id, 0);
}
...
static void nbl_flow_del_multi_rule(void *priv, u16 vsi)
{
	...
	NBL_FLOW_MCC_INDEX_KEY_INIT(&index_key, NBL_MCC_INDEX_VSI, vsi);
	mcc_id = nbl_common_get_index(flow_mgt->mcc_tbl_priv, &index_key, NULL);
	nbl_common_free_index(flow_mgt->mcc_tbl_priv, &index_key);
...
static int nbl_flow_add_multi_group(struct nbl_resource_mgt *res_mgt, u8 eth)
{
	...
	struct nbl_flow_mcc_index_key index_key = {0};
	struct nbl_flow_param param_down = {0}, param_up = {0};
	struct nbl_flow_mcc_node *mcc_node, *mcc_node_safe;
	int i, ret;

	/* The structure for MCC list is:
	 *
	 *    l2/l3_mc_up
	 *         |
	 *         |
	...
	param_down.mcc_id = nbl_common_get_index(flow_mgt->mcc_tbl_priv, &index_key, NULL);
	param_down.eth = eth;

	NBL_FLOW_MCC_INDEX_KEY_INIT(&index_key, NBL_MCC_INDEX_BMC, eth);
	param_up.mcc_id = nbl_common_get_index(flow_mgt->mcc_tbl_priv, &index_key, NULL);
	param_up.eth = eth;

	multi_group = &flow_mgt->multi_flow[eth];
	for (i = 0; i < NBL_FLOW_TYPE_MAX - NBL_FLOW_MACVLAN_MAX; i++) {
		if (i % 2)
			ret = nbl_flow_add_flow(res_mgt, param_down, i + NBL_FLOW_MACVLAN_MAX,
						&multi_group->entry[i]);
		else
			ret = nbl_flow_add_flow(res_mgt, param_up, i + NBL_FLOW_MACVLAN_MAX,
						&multi_group->entry[i]);
		if (ret)
			goto add_macvlan_fail;
	}

	ret = nbl_flow_add_mcc_node(multi_group, res_mgt, NBL_MCC_INDEX_BMC,
				    NBL_FLOW_MCC_BMC_DPORT, param_up.mcc_id, 1);
	if (ret)
		goto add_mcc_bmc_fail;

	ret = nbl_flow_add_mcc_node(multi_group, res_mgt, NBL_MCC_INDEX_ETH, eth,
				    param_down.mcc_id, 1);
	if (ret)
		goto add_mcc_eth_fail;

	multi_group->ether_id = eth;
	multi_group->mcc_id = param_up.mcc_id;

	return 0;

add_mcc_eth_fail:
	list_for_each_entry_safe(mcc_node, mcc_node_safe, &multi_group->mcc_head, node)
		nbl_flow_del_mcc_node(multi_group, res_mgt, mcc_node);
add_mcc_bmc_fail:
add_macvlan_fail:
	while (--i >= 0)
		nbl_flow_del_flow(res_mgt, &multi_group->entry[i]);
	return ret;
}
...
static void nbl_flow_del_multi_group(struct nbl_resource_mgt *res_mgt, u8 eth)
{
	...
	INIT_LIST_HEAD(&multi_group->mcc_head);
}
...
static void nbl_flow_del_ktls_rx_flow(void *priv, u32 index)
{
	...
	if (nbl_list_entry_is_head(rule, &flow_mgt->ul4s_head, node))
		return;
...
static void nbl_flow_del_ipsec_tx_flow(void *priv, u32 index)
{
	...
	if (nbl_list_entry_is_head(rule, &flow_mgt->dprbac_head, node))
		return;
...
static void nbl_res_flow_del_nd_upcall_flow(void *priv)
{
	...
	if (nbl_list_entry_is_head(rule, &flow_mgt->nd_upcall_list, node))
		return;
...
do {											\
	...
	NBL_FLOW_SET_OPS(cfg_lag_mcc, nbl_flow_cfg_lag_mcc);				\
...
static void nbl_flow_remove_mgt(struct device *dev, struct nbl_resource_mgt *res_mgt)
{
	...
	nbl_common_remove_index_table(flow_mgt->mcc_tbl_priv, NULL);
...
static int nbl_flow_setup_mgt(struct device *dev, struct nbl_resource_mgt *res_mgt)
{
	...
	for (i = 0; i < NBL_MAX_ETHERNET; i++) {
		...
		INIT_LIST_HEAD(&flow_mgt->multi_flow[i].mcc_head);
	}
