...
static u32 nbl_phy_dump_registers[] = {
	...
	NBL_USTORE_BUF_TOTAL_TRUNCATE_PKT
};
...
static int nbl_ustore_init(struct nbl_phy_mgt *phy_mgt, u8 eth_num)
{
	...
	for (i = 0; i < NBL_MAX_ETHERNET; i++) {
		...
		nbl_hw_rd32(phy_mgt, NBL_USTORE_BUF_PORT_TRUNCATE_PKT(i));
	}
...
static void nbl_phy_set_driver_status(struct nbl_phy_mgt *phy_mgt, bool active)
{
	u32 status = 0;

	status = nbl_hw_rd32(phy_mgt, NBL_DRIVER_STATUS_REG);

	status = (status & ~(1 << NBL_DRIVER_STATUS_BIT)) | (active << NBL_DRIVER_STATUS_BIT);

	nbl_hw_wr32(phy_mgt, NBL_DRIVER_STATUS_REG, status);
}

static void nbl_phy_deinit_chip_module(void *priv)
{
	struct nbl_phy_mgt *phy_mgt = (struct nbl_phy_mgt *)priv;

	nbl_phy_set_driver_status(phy_mgt, false);
}
...
static int nbl_phy_init_chip_module(void *priv, u8 eth_speed, u8 eth_num)
{
	...
	nbl_phy_set_driver_status(phy_mgt, true);
...
static int nbl_phy_get_ustore_pkt_drop_stats(void *priv, u8 eth_id,
					     struct nbl_ustore_stats *stats)
{
	struct nbl_phy_mgt *phy_mgt = (struct nbl_phy_mgt *)priv;

	stats->rx_drop_pkt = nbl_hw_rd32(phy_mgt, NBL_USTORE_BUF_PORT_DROP_PKT(eth_id));
	stats->rx_truncate_pkt = nbl_hw_rd32(phy_mgt, NBL_USTORE_BUF_PORT_TRUNCATE_PKT(eth_id));
...
static struct nbl_phy_ops phy_ops = {
	...
	.deinit_chip_module		= nbl_phy_deinit_chip_module,
