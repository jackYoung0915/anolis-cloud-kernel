				  ...
				  struct nbl_chan_info *chan_info)
{
	...
	if (!chan_info->wait) {
		dev_err(dev, "Allocate Txq wait_queue_head array failed\n");
		goto req_wait_queue_failed;
	}
	txq->buf = devm_kcalloc(dev, chan_info->num_txq_entries,
				sizeof(struct nbl_chan_buf), GFP_KERNEL);
	if (!txq->buf) {
		dev_err(dev, "Allocate memory for chan tx buffer arrays failed\n");
				  ...
				  struct nbl_chan_info *chan_info)
{
	...
	rxq->buf = devm_kcalloc(dev, chan_info->num_rxq_entries,
				sizeof(struct nbl_chan_buf), GFP_KERNEL);
	if (!rxq->buf) {
		dev_err(dev, "Allocate memory for chan rx buffer arrays failed\n");
...
static int nbl_chan_send_ack(void *priv, struct nbl_chan_ack_info *chan_ack)
{
	...
	if (!tmp) {
		dev_err(dev, "Chan send ack data malloc falied\n");
		return -ENOMEM;
	}
