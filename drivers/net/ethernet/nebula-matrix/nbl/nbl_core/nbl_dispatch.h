...
({												\
	...
	if (_disp_mgt->ops_lock_required)							\
		mutex_lock(&_disp_mgt->ops_muxtex_lock);					\
												\
	/* The following code can be understand in this way:
	 *
	 * if (func has void return value)
	 *     (!_func) ? 0 : _func(__VA_ARGS__);
	 * else
	 *     ret = (!_func) ? 0 : _func(__VA_ARGS__);
	 *
	 * __builtin_choose_expr is a gcc builtin function that equals to ternary operator.
	 *     __builtin_choose_expr(cond, expr1, expr2) == cond ? expr1 : expr2
	 */											\
	__builtin_choose_expr(									\
		 ...
		 *     ret = (!_func) ? 0 : _func(__VA_ARGS__)
		 * Because although __builtin_choose_expr only compile the selected branch,
		 * it still checks syntax error on the used branch, and the above example is
		 * illegal when func has void return.
		 * So we have to wrap a __builtin_choose_expr again, to avoid compiler to form
		 * a sentence as ret = _func(__VA_ARGS__) when return value is void
		 */										\
