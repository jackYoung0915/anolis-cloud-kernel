...
static int xsc_get_module_eeprom_by_page(struct net_device *netdev,
					 const struct ethtool_module_eeprom *page_data,
					 struct netlink_ext_ack *extack)
{
	struct xsc_adapter *priv = netdev_priv(netdev);
	struct xsc_core_device *xdev = priv->xdev;
	struct xsc_module_eeprom_query_params query;
	...
	while (i < page_data->length) {
		...
		size_read = xsc_query_module_eeprom_by_page(xdev, &query, data + i);
		...
		if (size_read == -EINVAL)
			return -EINVAL;
		if (size_read < 0) {
			netdev_err(priv->netdev, "%s: xsc_query_module_eeprom_by_page failed:0x%x\n",
				   __func__, size_read);
			return i;
		}
...
static int xsc_get_rxfh(struct net_device *netdev, struct ethtool_rxfh_param *rxfh)
{
	struct xsc_adapter *priv = netdev_priv(netdev);
...
static int xsc_set_rxfh(struct net_device *dev, struct ethtool_rxfh_param *rxfh,
			struct netlink_ext_ack *extack)
{
	struct xsc_adapter *priv = netdev_priv(dev);
	struct xsc_rss_params *rss = &priv->rss_params;
...
static const struct ethtool_ops xsc_ethtool_ops = {
	...
	.get_module_eeprom_by_page = xsc_get_module_eeprom_by_page,
