...
#else
#define FXGMAC_MSIX_CH0RXDIS_ENABLED \
	0 /* NO modification needed! for non-MSI, set to 0 always */
...
#else
#define FXGMAC_RSS_FEATURE_ENABLED 1
#endif
#define FXGMAC_RSS_HASH_KEY_LINUX \
	1 /* 0:hard to default rss key ;1: normal hash key process from Linux. */

/* WOL features */
#define FXGMAC_WOL_FEATURE_ENABLED 1
/* since wol upon link will cause issue, disabled it always. */
#define FXGMAC_WOL_UPON_EPHY_LINK \
	1 /* 1:enable ephy link change wol ; 0: ephy link change wol is not supported. */

/* Pause features */
#define FXGMAC_PAUSE_FEATURE_ENABLED \
	1 /* 1:enable flow control/pause framce ; 0: flow control/pause frame not included. */

/* ARP offload engine (AOE) */
#define FXGMAC_AOE_FEATURE_ENABLED \
	1 /* 1:enable arp offload engine ; 0: aoe is not included. */

/* NS offload engine */
#define FXGMAC_NS_OFFLOAD_ENABLED \
	1 /* 1:enable NS offload for IPv6 ; 0: NS is not included. */

/* for fpga ver after, which needs release phy before set of MAC tx/rx */
#define FXGMAC_TXRX_EN_AFTER_PHY_RELEASE \
	1 /* 1:release ephy before mac tx/rx bits are set. */

/*  power management features */
#define FXGMAC_PM_FEATURE_ENABLED 1

/* sanity check */
#define FXGMAC_SANITY_CHECK_ENABLED 0
...
#define cfg_r8(_pdata, reg, pdat) \
	pci_read_config_byte((_pdata)->pdev, (reg), (u8 *)(pdat))

/* read from 16bit register via pci config space */
#define cfg_r16(_pdata, reg, pdat) \
	pci_read_config_word((_pdata)->pdev, (reg), (u16 *)(pdat))

/* read from 32bit register via pci config space */
#define cfg_r32(_pdata, reg, pdat) \
	pci_read_config_dword((_pdata)->pdev, (reg), (u32 *)(pdat))

/* write to 8bit register via pci config space */
#define cfg_w8(_pdata, reg, val) \
	pci_write_config_byte((_pdata)->pdev, (reg), (u8)(val))

/* write to 16bit register via pci config space */
#define cfg_w16(_pdata, reg, val) \
	pci_write_config_word((_pdata)->pdev, (reg), (u16)(val))

/* write to 32bit register via pci config space */
#define cfg_w32(_pdata, reg, val) \
	pci_write_config_dword((_pdata)->pdev, (reg), (u32)(val))
