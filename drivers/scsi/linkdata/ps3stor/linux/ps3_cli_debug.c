...
static struct ps3_cli_debug_cmd g_ps3_cli_debug_cmd_table[] = {
	...
	"\t\t(1 timeout_force_reply/2 error_reply)\n"
	...
	{ ps3_inject_scsi_task, "inject_scsi_task",
	  "host_no xxx dev xxx xxx(chan id) abort/reset (3 timeout/4 error_reply) times xxx" },
	{ ps3_inject_mgr, "inject_mgr",
	  "host_no xxx(host number) cmd_type xxx cmd_sub_type xxx xxx\n"
		  "\t\t(7 timeout/8 error_reply xxx (errType)) times xxx" },
	...
	{ ps3_clear_inject_mgr, "clear_inject_mgr",
	  "host_no xxx(host number) cmd_type xxx cmd_sub_type xxx" },
	{ ps3_clear_all_inject, "clear_inject_all", "clear_inject_all" },
	{ ps3_show_all_inject, "show_inject_all", "show_inject_all" },
	{ ps3_clear_hit_cmd, "ps3_clear_hit_cmd", "ps3_clear_hit_cmd" },
#endif
};
...
struct inject_cmds_t *ps3_inject_scsi_rw_exist(struct inject_cmds_t *this_pitem)
{
	...
	struct PS3Inject *p_inject_list = get_inject();
	unsigned char is_find = PS3_FALSE;
	...
	list_for_each_entry(pitem, &p_inject_list->scsi_rw_list, list) {
		if ((this_pitem->item.scsi_cmd.host_no ==
		     pitem->item.scsi_cmd.host_no) &&
		    (this_pitem->item.scsi_cmd.channel ==
		     pitem->item.scsi_cmd.channel) &&
		    (this_pitem->item.scsi_cmd.id == pitem->item.scsi_cmd.id)) {
			if (((this_pitem->item.scsi_cmd.lba >=
			      pitem->item.scsi_cmd.lba) &&
			     (this_pitem->item.scsi_cmd.lba <=
			      pitem->item.scsi_cmd.lba +
				      pitem->item.scsi_cmd.len)) ||
			    ((this_pitem->item.scsi_cmd.lba +
				      this_pitem->item.scsi_cmd.len >=
			      pitem->item.scsi_cmd.lba) &&
			     (this_pitem->item.scsi_cmd.lba +
				      this_pitem->item.scsi_cmd.len <=
			      pitem->item.scsi_cmd.lba +
				      pitem->item.scsi_cmd.len))) {
				ps3stor_cli_printf(
					"inject [%u:%u:%u] lba[0x%llx], len[0x%x], is exist!\n",
					pitem->item.scsi_cmd.host_no,
					pitem->item.scsi_cmd.channel,
					pitem->item.scsi_cmd.id,
					pitem->item.scsi_cmd.lba,
					pitem->item.scsi_cmd.len);
				is_find = PS3_TRUE;
				break;
...
ps3_inject_scsi_task_exist(struct inject_cmds_t *this_pitem)
{
	struct inject_cmds_t *pitem = NULL;
	struct PS3Inject *p_inject_list = get_inject();
	unsigned char is_find = PS3_FALSE;
	...
	list_for_each_entry(pitem, &p_inject_list->scsi_task_list, list) {
		if ((this_pitem->item.scsi_task_cmd.host_no ==
		     pitem->item.scsi_task_cmd.host_no) &&
		    (this_pitem->item.scsi_task_cmd.channel ==
		     pitem->item.scsi_task_cmd.channel) &&
		    (this_pitem->item.scsi_task_cmd.id ==
		     pitem->item.scsi_task_cmd.id)) {
			if (this_pitem->item.scsi_task_cmd.cmd_type ==
				    pitem->item.scsi_task_cmd.cmd_type &&
			    this_pitem->item.scsi_task_cmd.cmd_sub_type ==
				    pitem->item.scsi_task_cmd.cmd_sub_type) {
				is_find = PS3_TRUE;
				break;
			}
		}
...
struct inject_cmds_t *ps3_inject_mgr_exist(struct inject_cmds_t *this_pitem)
{
	...
	list_for_each_entry(pitem, &p_inject_list->mgr_list, list) {
		if ((this_pitem->item.mgr_cmd.host_no ==
		     pitem->item.mgr_cmd.host_no) &&
		    (this_pitem->item.mgr_cmd.cmd_type ==
		     pitem->item.mgr_cmd.cmd_type) &&
		    (this_pitem->item.mgr_cmd.cmd_sub_type ==
		     pitem->item.mgr_cmd.cmd_sub_type)) {
			is_find = PS3_TRUE;
		}
	}
l_out:
...
void ps3_show_inject_scsi_rw_item(struct inject_cmds_t *pitem)
{
	...
	pitem1 = ps3_inject_scsi_rw_exist(pitem);
	if (pitem1 == NULL) {
		ps3stor_cli_printf("rw inject type is noexist!\n");
		goto l_out;
	}
	ps3stor_cli_printf(
		"inject_rw host_no %u channel %u id %u lba 0x%llx len %u type %u, time %u!\n",
...
void ps3_show_inject_all_scsi_rw(int argc, char *argv[])
{
	...
	list_for_each_entry(pitem, &p_inject_list->scsi_rw_list, list) {
		ps3stor_cli_printf(
			"inject_rw host_no %u channel %u id %u lba 0x%llx len %u!\n",
			pitem->item.scsi_cmd.host_no,
			pitem->item.scsi_cmd.channel, pitem->item.scsi_cmd.id,
...
void ps3_show_inject_scsi_task_item(struct inject_cmds_t *pitem)
{
	...
	ps3_mutex_unlock(&p_inject_list->lock);
	pitem1 = ps3_inject_scsi_task_exist(pitem);

	if (pitem1 == NULL) {
		ps3stor_cli_printf("task inject type is noexist!\n");
		goto l_out;
	}
	ps3stor_cli_printf("inj_rw hno %u ch %u id %u ctype %u cSubtype %u dType %u times %u!\n",
			   pitem1->item.scsi_task_cmd.host_no,
			   pitem1->item.scsi_task_cmd.channel,
			   pitem1->item.scsi_task_cmd.id,
			   pitem1->item.scsi_task_cmd.cmd_type,
			   pitem1->item.scsi_task_cmd.cmd_sub_type,
			   pitem1->item.scsi_task_cmd.dealType,
...
void ps3_show_inject_mgr_item(struct inject_cmds_t *pitem)
{
	...
	ps3_mutex_unlock(&p_inject_list->lock);
	pitem1 = ps3_inject_mgr_exist(pitem);
	...
	ps3stor_cli_printf(
		"inject_mgr host_no %u cmdtype %u cmdSubtype %u dealType %u times %u!\n",
		pitem1->item.mgr_cmd.host_no, pitem1->item.mgr_cmd.cmd_type,
		pitem1->item.mgr_cmd.cmd_sub_type,
		pitem1->item.mgr_cmd.dealType,
...
void ps3_show_inject_all_scsi_task(int argc, char *argv[])
{
	...
	list_for_each_entry(pitem, &p_inject_list->scsi_task_list, list) {
		ps3stor_cli_printf(
		"inj_task hno %u chani %u id %u ctype %u cSubtype %u dType %u times %u!\n",
		pitem->item.scsi_task_cmd.host_no,
		pitem->item.scsi_task_cmd.channel,
		pitem->item.scsi_task_cmd.id,
		pitem->item.scsi_task_cmd.cmd_type,
...
void ps3_show_inject_all_mgr(int argc, char *argv[])
{
	...
	list_for_each_entry(pitem, &p_inject_list->mgr_list, list) {
		ps3stor_cli_printf("inj_task hno %u ctype %u cSubtype %u dType %u times %u!\n",
				   pitem->item.mgr_cmd.host_no,
				   pitem->item.mgr_cmd.cmd_type,
				   pitem->item.mgr_cmd.cmd_sub_type,
...
void ps3_clear_all_inject_scsi_rw(void)
{
	...
	list_for_each_entry_safe(pitem, pitem_next,
				  &p_inject_list->scsi_rw_list, list) {
		list_del(&pitem->list);
		kfree(pitem);
	}
...
void ps3_clear_all_inject_scsi_task(void)
{
	...
	list_for_each_entry_safe(pitem, pitem_next,
				&p_inject_list->scsi_task_list, list) {
		list_del(&pitem->list);
		kfree(pitem);
	}
...
void ps3_clear_all_inject_mgr(void)
{
	...
	list_for_each_entry_safe(pitem, pitem_next, &p_inject_list->mgr_list,
		list) {
		list_del(&pitem->list);
		kfree(pitem);
	}
...
void ps3_del_inject_scsi_rw(struct inject_cmds_t *pitem)
{
	...
	ps3_mutex_lock(&p_inject_list->lock);
	ps3stor_cli_printf(
		"del inject_rw host_no %u channel %u id %u lba 0x%llx len %u type %u, time %u!\n",
		pitem1->item.scsi_cmd.host_no, pitem1->item.scsi_cmd.channel,
		pitem1->item.scsi_cmd.id, pitem1->item.scsi_cmd.lba,
		pitem1->item.scsi_cmd.len, pitem1->item.scsi_cmd.dealType,
		pitem1->item.scsi_cmd.inject_count);
...
void ps3_del_inject_scsi_task(struct inject_cmds_t *pitem)
{
	...
	ps3_mutex_lock(&p_inject_list->lock);
	ps3stor_cli_printf(
		"del inj_task hno %u chan %u id %u ctype %u cSubtype %u dType %u times %u!\n",
		pitem1->item.scsi_task_cmd.host_no,
		pitem1->item.scsi_task_cmd.channel,
		pitem1->item.scsi_task_cmd.id,
		pitem1->item.scsi_task_cmd.cmd_type,
...
void ps3_del_inject_mgr(struct inject_cmds_t *pitem)
{
	...
	ps3_mutex_lock(&p_inject_list->lock);
	ps3stor_cli_printf(
		"del inject_mgr host_no %u cmdtype %u cmdSubtype %u dealType %u times %u!\n",
		pitem1->item.mgr_cmd.host_no, pitem1->item.mgr_cmd.cmd_type,
		pitem1->item.mgr_cmd.cmd_sub_type,
		pitem1->item.mgr_cmd.dealType,
		pitem1->item.mgr_cmd.inject_count);
...
int ps3_add_inject_scsi_rw(struct inject_cmds_t *pitem)
{
	int iRet = PS3_SUCCESS;
	struct PS3Inject *p_inject_list = get_inject();
...
int ps3_add_inject_scsi_task(struct inject_cmds_t *pitem)
{
	int iRet = PS3_SUCCESS;
	struct PS3Inject *p_inject_list = get_inject();
	...
	ps3_mutex_lock(&p_inject_list->lock);
	ps3stor_cli_printf(
		"inj_task hno %u chan %u id %u ctype %u cSubtype %u dType %u times %u!\n",
		pitem->item.scsi_task_cmd.host_no,
		pitem->item.scsi_task_cmd.channel, pitem->item.scsi_task_cmd.id,
		pitem->item.scsi_task_cmd.cmd_type,
		pitem->item.scsi_task_cmd.cmd_sub_type,
		pitem->item.scsi_task_cmd.dealType,
		pitem->item.scsi_task_cmd.inject_count);
...
int ps3_add_inject_mgr(struct inject_cmds_t *pitem)
{
	int iRet = PS3_SUCCESS;
	struct PS3Inject *p_inject_list = get_inject();
	...
	ps3_mutex_lock(&p_inject_list->lock);
	ps3stor_cli_printf(
		"inject_task host_no %u cmdtype %u cmdSubtype %u dealType %u times %u!\n",
...
static void ps3_inject_scsi_rw(int argc, char *argv[])
{
	...
	pitem = kmalloc(sizeof(struct inject_cmds_t), GFP_KERNEL);
	if (pitem == NULL)
		return;
	...
	case PS3_SCSI_CMD_ERROR:
		...
		pitem->item.scsi_cmd.cmdDeal.errReply.sshdr.resp_code =
			sense_info & 0xf;
		pitem->item.scsi_cmd.cmdDeal.errReply.sshdr.sense_key =
			sense_info >> 8 & 0xf;
		pitem->item.scsi_cmd.cmdDeal.errReply.sshdr.asc =
			sense_info >> 16 & 0xf;
		pitem->item.scsi_cmd.cmdDeal.errReply.sshdr.ascq =
			sense_info >> 24 & 0xf;
...
static void ps3_inject_scsi_task(int argc, char *argv[])
{
	...
	if (dealType != PS3_SCSI_TASK_CMD_TIMEOUT &&
	    dealType != PS3_SCSI_TASK_CMD_ERROE &&
...
void ps3_scsi_task_cmd_clear_all(void)
{
	...
	struct inject_cmds_t *pitem = NULL;
	struct PS3Inject *p_inject_list = get_inject();
	struct PS3HitCmd *p_hit_inject = get_hit_inject();
	...
	list_for_each_entry_safe(p_cmd, p_cmd_next,
				  &p_hit_inject->scsi_task_list, list) {
		list_del(&p_cmd->list);
		kfree(p_cmd);
	}
...
void ps3_mgr_cmd_clear_all(void)
{
	...
	list_for_each_entry_safe(p_cmd, p_cmd_next, &p_hit_inject->mgr_list,
				  list) {
		list_del(&p_cmd->list);
		kfree(p_cmd);
	}
...
void ps3_scsi_rw_cmd_clear_all(void)
{
	...
	list_for_each_entry_safe(p_cmd, p_cmd_next,
				  &p_hit_inject->scsi_rw_list, list) {
		list_del(&p_cmd->list);
