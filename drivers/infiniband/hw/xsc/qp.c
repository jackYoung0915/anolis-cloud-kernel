...
{
	u32 wqe_cnt = roundup_pow_of_two(cap->max_recv_wr);
			...
			struct xsc_ib_qp *qp)
{
	...
	qp->sq.wqe_cnt = min_t(int, qp->sq.wqe_cnt, (int)dev->xdev->caps.max_wqes);
		     ...
		     struct ib_udata *udata)
{
	...
	if (init_attr->qp_type != IB_QPT_RAW_PACKET) {
		if (!is_support_rdma(dev->xdev) ||
			...
			return RET_VALUE(-EPROTONOSUPPORT);
		}
	}
...
void xsc_ib_drain_rq(struct ib_qp *qp __maybe_unused)
{
}

void xsc_ib_drain_sq(struct ib_qp *qp __maybe_unused)
{
}
