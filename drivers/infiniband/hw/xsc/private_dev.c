...
DECLARE_BITMAP(g_bitmap_cdev_id, XSC_MAX_CDEV_NUM);
...
int xsc_priv_dev_init(struct ib_device *ib_dev, struct xsc_core_device *dev)
{
	...
	dev_id = find_first_zero_bit(g_bitmap_cdev_id, XSC_MAX_CDEV_NUM);
	...
	g_priv_cdev_cnt++;
	set_bit(dev_id, g_bitmap_cdev_id);
...
void xsc_priv_dev_fini(struct ib_device *ib_dev, struct xsc_core_device *dev)
{
	...
	clear_bit(dev_id, g_bitmap_cdev_id);
	g_priv_cdev_cnt--;
