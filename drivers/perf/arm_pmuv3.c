...
#define HISI_DTU_CTLR_EL1_CHK_GROUP0	BIT(15)
	...
	/* Readback to ensure write completion (acts as a barrier) */
	reg = read_sysreg_s(HISI_DTU_CTLR_EL1);
}
#endif
					  ...
					  struct perf_event *event)
{
	...
	/* Both events in a hw_metric pair must be hw_metric */
	...
	if (cpuc->percpu_pmu) {
		...
		/*
		 * Exclude the current event from counting to avoid
		 * self-counting during re-validation in validate_group.
		 * The new event being added is not in sibling_list yet,
		 * so this exclusion only affects existing members.
		 */
		for_each_sibling_event(sibling, leader) {
			if (sibling != event &&
				armv8pmu_event_is_hw_metric(sibling))
				hw_metric_cnt++;
		}
					    ...
					    struct perf_event *event)
{
	...
	if (event == leader || leader->hw.idx < 1)
		/*
		 * HIP12 has at most 6 general counters (idx 1-6). get_chain_idx returns
		 * an even index within this range, ensuring hw.idx <= 6.
		 */
