...
static struct iommu_domain *ummu_domain_alloc_paging_flags(struct device *dev,
...
static int ummu_attach_dev(struct iommu_domain *domain, struct device *dev, struct iommu_domain *old)
{
	struct ummu_domain *u_domain = to_ummu_domain(domain);
	struct ummu_master *master =
		(struct ummu_master *)dev_iommu_priv_get(dev);
...
struct iommu_ops ummu_iommu_ops = {
	...
	.domain_alloc_paging_flags_v2 = ummu_domain_alloc_paging_flags,
