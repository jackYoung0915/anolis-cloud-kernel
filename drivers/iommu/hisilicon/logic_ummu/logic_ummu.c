				 ...
				 struct device *dev, struct iommu_domain *old)
{
	...
	ret = ops->attach_dev(&agent_domain->domain, dev, NULL);
	if (ret) {
...
static struct iommu_domain *
logic_ummu_domain_alloc_paging_flags_v2(struct device *dev, u32 flags,
			     struct kvm *kvm, const struct iommu_user_data *data)
{
	const struct iommu_ops *ops = get_agent_iommu_ops();
	struct ummu_base_domain *base_domain, *next;
	struct logic_ummu_domain *logic_domain;
	...
	if (!ops || !ops->domain_alloc_paging_flags_v2)
		return ERR_PTR(-EOPNOTSUPP);
	logic_domain = kzalloc(sizeof(*logic_domain), GFP_KERNEL);
	if (!logic_domain)
		return ERR_PTR(-ENOMEM);
	INIT_LIST_HEAD(&logic_domain->base_domain.list);
	list_for_each_entry(ummu, &logic_ummu.dev_list, list) {
		domain = ops->domain_alloc_paging_flags_v2(dev, flags, kvm, data);
		if (IS_ERR(domain)) {
			ret = PTR_ERR(domain);
			goto error_handle;
		}
...
static size_t logic_ummu_get_viommu_size(struct device *dev,
					 enum iommu_viommu_type viommu_type)
{
	...
	return VIOMMU_STRUCT_SIZE(struct logic_ummu_viommu, viommu);
}

static int logic_ummu_viommu_init(struct iommufd_viommu *viommu,
				  struct iommu_domain *parent,
				  const struct iommu_user_data *user_data)
{
	struct logic_ummu_viommu *logic_vummu;

	logic_vummu = container_of(viommu, struct logic_ummu_viommu, viommu);
	logic_vummu->core_dev = container_of(viommu->iommu_dev,
					     struct ummu_core_device, iommu);
	logic_vummu->parent = parent;
	viommu->ops = &logic_ummu_viommu_ops;
...
static int logic_ummu_of_xlate(struct device *dev, const struct of_phandle_args *args)
{
	const struct iommu_ops *ops = get_agent_iommu_ops();
...
static struct iommu_ops logic_iommu_ops = {
	.get_viommu_size = logic_ummu_get_viommu_size,
	.viommu_init = logic_ummu_viommu_init,
	.pgsize_bitmap = SZ_4K,
	.owner = THIS_MODULE,
};
...
static void gen_iommu_ops(const struct iommu_ops *src, struct iommu_ops *dst)
{
	...
	__GEN_OPS(domain_alloc_paging_flags_v2, src, dst);
