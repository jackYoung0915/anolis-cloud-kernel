...
static __always_inline void get_hook_msg(struct packet *p,
...
TRACEPOINT_PROBE(drivers, RDMA_sendmsg)
{
	struct packet p;
	uint32_t portpair;
	uint64_t addrpair;
...
TRACEPOINT_PROBE(smc_msg_event, smc_send_complete)
{
	...
	get_hook_msg(&p, "smc_send_complete",
