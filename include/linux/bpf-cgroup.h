...
/*
 * For KABI reason, we should keep struct cgroup_bpf always the same.
 * The upstream MAX_CGROUP_BPF_ATTACH_TYPE now is 33, so the space is enough.
 */
static_assert(MAX_CGROUP_BPF_ATTACH_TYPE <= ANOLIS_KABI_MAX_CG_BPF_ATTACH);
...
struct cgroup_bpf {
	...
	struct bpf_prog_array __rcu *effective[ANOLIS_KABI_MAX_CG_BPF_ATTACH];
	 ...
	 */
	struct list_head progs[ANOLIS_KABI_MAX_CG_BPF_ATTACH];
	u32 flags[ANOLIS_KABI_MAX_CG_BPF_ATTACH];
...
#define BPF_CGROUP_RUN_PROG_INET4_BIND(sk, uaddr)			       \
	BPF_CGROUP_RUN_SA_PROG(sk, uaddr, CGROUP_INET4_BIND)

#define BPF_CGROUP_RUN_PROG_INET6_BIND(sk, uaddr)			       \
	BPF_CGROUP_RUN_SA_PROG(sk, uaddr, CGROUP_INET6_BIND)
