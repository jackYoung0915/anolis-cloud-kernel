...
int (*amd_pmu_branch_hw_config)(struct perf_event *event) = NULL;
...
static int amd_core_hw_config(struct perf_event *event)
{
	...
	if (has_branch_stack(event))
		return amd_pmu_branch_hw_config(event);
...
static int __init amd_core_pmu_init(void)
{
	...
	if (!amd_pmu_lbr_init()) {
		...
		amd_pmu_branch_hw_config = amd_pmu_lbr_hw_config;
		...
		amd_pmu_branch_hw_config = amd_brs_hw_config;
