...
static void __init csv3_reserve_mem_block(void)
{
	...
	if (size_cmdline % align) {
		...
		pr_warn("csv3_guest_rsv_mb: size must be multiple of %u MB, "
			"align up to %lu MB\n",
	...
	csv3_mem_block = memblock_alloc_node(meta_size, SMP_CACHE_BYTES,
					     NUMA_NO_NODE);
	if (!csv3_mem_block) {
		pr_err("csv3_guest_rsv_mb: fail to allocate size 0x%lx from "
			"memblock\n",
			meta_size);
		return;
	}
	...
	while (reserved_size < csv3_mem_size) {
		...
		pr_debug("csv3_guest_rsv_mb: reserve memblock[%ld] 0x%lx MB "
			"memory, pa 0x%llx\n",
...
fail_free:
	...
	memblock_free(__pa(csv3_mem_block), meta_size);
	csv3_mem_block = NULL;
}
...
static bool decrypt_mem_block(void)
{
	...
	for (i = 0; i < csv3_mem_block->nr_blocks; i++) {
		...
		if (set_memory_decrypted(virt, size >> PAGE_SHIFT)) {
			pr_err("csv3_guest_rsv_mb: decrypt pa 0x%llx "
			       "size 0x%lx va 0x%llx failed\n",
...
void csv3_free_mem_block(phys_addr_t phys_addr)
{
	...
	if (csv3_mem_block->nr_free >= csv3_mem_block->nr_blocks) {
		mutex_unlock(&lock);
		pr_err("csv3_guest_rsv_mb: all blocks are free, "
			"please check\n");
		return;
	}
	...
	if (free_done) {
		...
		pr_err("csv3_guest_rsv_mb: fail to free 0x%lx MB memory, "
		       "pa 0x%llx\n",
		       csv3_mem_block->block_size / SZ_1M, phys_addr);
	}
}
