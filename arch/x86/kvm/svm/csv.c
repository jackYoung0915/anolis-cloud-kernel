 ...
 * hugetlb.
 */
static int csv3_alloc_1G_hugetlb_folios(struct kvm *kvm,
					unsigned long size,
					nodemask_t *nodemask)
{
	struct kvm_csv_info *csv = &to_kvm_svm_csv(kvm)->csv_info;
	...
	for (i = 0; i < huge_count; i++) {
		...
		folio = dequeue_1G_hugetlb_folio_nodemask(GFP_KERNEL,
							  NUMA_NO_NODE, nodemask);
		if (!folio) {
			kfree(hugetlb);
			ret = -ENOMEM;
			goto err;
...
int csv_set_msr(struct kvm_vcpu *vcpu, struct msr_data *msr_info)
{
	...
	case MSR_AMD64_SEV_ES_GHCB:
		...
		/* Ignore write to this MSR if it's not a Hygon CSV2 guest. */
		if (!sev_es_guest(svm->vcpu.kvm))
			return 0;
