...
static int sev_asid_new(bool es_active, const char *userid, u32 userid_len)
{
	...
	if ((boot_cpu_data.x86_vendor == X86_VENDOR_HYGON) &&
	    userid && userid_len) {
		i = (!min_sev_asid) ? 0 : min_sev_asid - 1;
...
again:
	...
	if ((boot_cpu_data.x86_vendor == X86_VENDOR_HYGON) &&
	    userid && userid_len) {
		memcpy(csv_asid_userid_array[pos].userid, userid, userid_len);
		csv_asid_userid_array[pos].userid_len = userid_len;
		csv_asid_userid_array[pos].refcnt = 1;
	}
...
static void sev_asid_free(int asid)
{
	...
	if ((boot_cpu_data.x86_vendor == X86_VENDOR_HYGON) &&
	    csv_asid_userid_array[pos].userid_len) {
		/* If reach here, reference count should large than 0. */
		WARN_ON(csv_asid_userid_array[pos].refcnt <= 0);

		if (--csv_asid_userid_array[pos].refcnt == 0) {
			__set_bit(pos, sev_reclaim_asid_bitmap);

			memset(&csv_asid_userid_array[pos], 0,
...
static int sev_guest_init(struct kvm *kvm, struct kvm_sev_cmd *argp)
{
	...
	if (boot_cpu_data.x86_vendor == X86_VENDOR_HYGON) {
		...
		asid = sev_asid_new(es_active, (const char *)csv_blob, params.len);
	} else {
		asid = sev_asid_new(es_active, NULL, 0);
	}
...
void __init sev_hardware_setup(void)
{
	...
	if (boot_cpu_data.x86_vendor == X86_VENDOR_HYGON) {
		...
		csv_asid_userid_array = kcalloc(max_sev_asid,
...
void pre_sev_run(struct vcpu_svm *svm, int cpu)
{
	...
	if ((boot_cpu_data.x86_vendor == X86_VENDOR_HYGON) &&
	    csv_asid_userid_array[asid - 1].userid_len)
