 ...
 * remapping process of this range can not be done becuase of inaccessible.
			   ...
			   int flags)
{
	...
	do {
		 ...
		 * physical address of pudp is included in the range it points
		 * to, split the block of pudp earlier.
		 */
...
{
	unsigned long val, tmp_size;
	int ret = kstrtoul(arg, 0, &val);

	if (ret < 0)
		return ret;

	tmp_size = (val + 1) * 2 * PAGE_SIZE;

	if (val > 0 && (tmp_size < PUD_SIZE)) {
		kfence_pool_size = tmp_size;
		kfence_num_objects = val;
	}

	return 0;
}
early_param("kfence.num_objects", parse_kfence_early_init_num_objects);
...
static void __init map_mem(pgd_t *pgdp)
{
	...
	if (crash_mem_map) {
		if ((IS_ENABLED(CONFIG_ZONE_DMA) ||
		    IS_ENABLED(CONFIG_ZONE_DMA32)) && !early_reserve_crashmem)
			flags |= NO_BLOCK_MAPPINGS | NO_CONT_MAPPINGS;
