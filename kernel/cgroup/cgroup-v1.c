				  ...
				  char *buf, size_t nbytes, loff_t off)
{
	...
	guard(rcu)();
	cgrp = rcu_dereference(of->kn->__parent)->priv;

	if (kstrtoull(buf, 0, &val))
		return -EINVAL;

	if (val > CGROUP_POOL_MAX)
		return -EINVAL;
	...
	if (val) {
		...
		for (i = 0; i < val;) {
			snprintf(name, sizeof(name), "pool-%llu",
				 atomic64_add_return(1, &cgrp->pool_index));
		...
		hidden_root = &(kernfs_root_to_node(cgrp->hidden_place)->dir.children);
