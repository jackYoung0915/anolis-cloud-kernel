...
static bool __kfence_init_pool_area(struct kfence_pool_area *kpa)
{
	...
	for (i = 0; i < kpa->pool_size / PAGE_SIZE; i++) {
		struct page *page = virt_to_page((void *)(addr + i * PAGE_SIZE));
...
fail:
	memblock_free(__kfence_pool, pool_size);
	__kfence_pool_area[index] = NULL;
	kfree(kpa);
...
fail_alloc:
	for_each_node(node) {
		for (area = 0; area < kfence_nr_areas_per_node; area++) {
			...
			if (__kfence_pool_area[index]) {
				memblock_free(__kfence_pool_area[index],
						   kfence_pool_size);
				__kfence_pool_area[index] = NULL;
			}
		}
...
out:
	memblock_free(__kfence_pool_area, sizeof(char *) * nr_node_ids *
