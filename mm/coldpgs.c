...
/* kidled is not implemented yet, add function stub */
			  ...
			  struct folio *folio)
{
	 ...
	 */
	pgoff = page_to_pgoff(folio_page(folio, 0));
			...
			bool validate_age)
{
	 ...
	 */
	if (validate_age) {
		age = kidled_get_page_age(pgdat, folio_pfn(folio));
		   ...
		   int age)
{
	...
	if (folio_clear_dirty_for_io(folio)) {
		...
		if (folio_test_anon(folio))
			ret = swapout_page(filter, &folio->page, age,
					   &wbc, &use_zswap);
		else
...
static void my_putback_inactive_pages(struct lruvec *lruvec, struct list_head *folios_list)
{
				...
				enum lru_list lru, struct list_head *list)
{
	...
	while (!list_empty(list)) {
		...
		if (folio_test_anon(folio) && folio_test_swapbacked(folio)) {
			if (!folio_test_swapcache(folio)) {
				...
				if (folio_test_large(folio)) {
					...
					if (data_race(!list_empty(&folio->_deferred_list) &&
					    ...
					    my_split_huge_page_to_list(&folio->page, list))
						goto keep_unlocked;
				}
				...
				if (!my_add_to_swap(folio)) {
					...
					if (my_split_huge_page_to_list(&folio->page,
								       list))
						goto keep_unlocked;
			...
			if (my_split_huge_page_to_list(&folio->page, list))
				goto keep_unlocked;
		}
...
free_it:
		...
		nr_reclaimed += nr_pages;
		kidled_mem_cgroup_move_stats(memcg, NULL, folio_page(folio, 0), nr_pages);
		if (is_pagecache)
...
keep:
	...
	while (!list_empty(list)) {
		...
		if (filter->batch && ++batch >= filter->batch) {
			spin_lock_irq(&lruvec->lru_lock);
			my_putback_inactive_pages(lruvec, &free_folios);
			spin_unlock_irq(&lruvec->lru_lock);
