...
mem_cgroup_css_alloc(struct cgroup_subsys_state *parent_css)
{
	...
	if (parent) {
		...
		WRITE_ONCE(memcg->wmark_min_adj, READ_ONCE(parent->wmark_min_adj));
		WRITE_ONCE(memcg->wmark_min_eadj, READ_ONCE(parent->wmark_min_eadj));
				   ...
				   struct mem_cgroup *to)
{
	...
	kidled_mem_cgroup_move_stats(from, to, folio, nr_pages);
