...
skip:
		if (count) {
			...
			add_mm_counter(vmf->vma->vm_mm, mm_counter_file(page),
				       count);
	...
	if (count) {
		set_pte_range(vmf, folio, page, count, addr);
		add_mm_counter(vmf->vma->vm_mm, mm_counter_file(page), count);
		folio_ref_add(folio, count);
		if (in_range(vmf->address, addr, count * PAGE_SIZE))
			ret = VM_FAULT_NOPAGE;
	}
		...
		unsigned int *mmap_miss)
{
	...
	add_mm_counter(vmf->vma->vm_mm, mm_counter_file(page), 1);
	folio_ref_inc(folio);
