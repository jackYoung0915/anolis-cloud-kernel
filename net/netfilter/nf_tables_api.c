...
/* Only called from commit path, nft_setelem_data_deactivate() already deals with
...
{
	struct netns_nftables *net_nft = &trans->net->nft;
	struct nft_ctx ctx = {};

	mutex_lock(&net_nft->commit_mutex);
	 ...
	 * worker loses race.
	 */
	if (READ_ONCE(net_nft->gc_seq) != trans->seq || trans->set->dead) {
		mutex_unlock(&net_nft->commit_mutex);
		return false;
	}
