...
struct sk_buff_nettrace *get_cached_skb(struct trace_point *tp)
{
	if (!tp->nr_skb_objs)
		return NULL;
...
bool put_cached_skb(struct trace_point *tp, struct sk_buff_nettrace *skb)
{
...
u8 *get_cached_data(struct trace_point *tp)
{
	if (!tp->nr_data_objs)
		return NULL;
...
bool put_cached_data(struct trace_point *tp, u8 *data)
{
...
void init_dump_mm(struct trace_point *tp)
{
	int i;
...
void release_dump_mm(struct trace_point *tp)
{
	int i;
	struct sk_buff_nettrace *skb;
	u8 *data;
...
static struct sk_buff_nettrace *__alloc_skb_nettrace(struct trace_point *tp)
{
	struct sk_buff_nettrace *skb;
	u8 *data;
...
void release_skb_nettrace(struct sk_buff_nettrace *skb, struct trace_point *tp)
{
...
struct sk_buff_nettrace *skb_copy_nettrace(const struct sk_buff *skb, struct trace_point *tp)
{
