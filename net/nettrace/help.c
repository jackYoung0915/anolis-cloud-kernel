...
MODULE_PARM_DESC(trace, "a trace is a serial of kernel tracing for special scene,We support various of trace, such as ip,upd,tcp,macvlan,link,link_input,link_output\n \
		e.g. insmod nettrace.ko trace=link");
PARAM_STRING_ARRAY(filter_trace, trace, 10)
MODULE_PARM_DESC(probe, "this is a list of kernel function where you want to dump network package info. Use 'probe=?' to see all supported kernel functions");
...
MODULE_PARM_DESC(output, "three kind of output supported: ftrace, kernel and file,When comes up with file, it should be a file path, such as /ntrace.log\n \
		e.g. insmod nettrace.ko output=ftrace");
PARAM_STRING_NAMED(print_output, output, NULL)
MODULE_PARM_DESC(dump, "the directory where you want to put pcap file in. Once thisoption is set, all package filtered will be saved\n \
		e.g. insmod nettrace.ko trace=error dump=./output");
...
static void print_enqueue(char *fmt, va_list ap)
{
	...
	strncpy(print_buf, buf, strlen(buf));

	print_file = kcalloc(1, sizeof(PRINT_ENTRY), GFP_KERNEL);
	if (!print_file) {
		kfree(print_buf);
		return;
	}
...
static void print_group(TRACE_GROUP *tg)
{
	TRACE_GROUP	*tmp_tg;
	char	tab[] = "    ";
	...
	if (cur_len + sizeof(tab) > sizeof(output_index))
		return;
	strncpy(output_index + cur_len, tab, strlen(tab));
...
static int init_rule(void)
{
	...
	if (filter_proto) {
		if ((proto = str2proto3(filter_proto)) >= 0) {
			SET_RULE_FLAGS(rule, proto_3, proto);
		} else if ((proto = str2proto4(filter_proto)) >= 0) {
			SET_RULE_FLAGS(rule, proto_4, proto);
		} else {
