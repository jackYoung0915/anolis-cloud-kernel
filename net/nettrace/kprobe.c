...
int filter_syms(void *data, const char *name_buf, unsigned long address)
{
	...
	if (strstarts(name_buf, name)) {
		...
		strncpy(name, name_buf, MAX_TP_NAME);
...
int c_register_kprobe(struct kprobe *p)
{
	if (register_kprobe(p) < 0) {
		...
		strncpy(name, p->symbol_name, sizeof(name) - 1);
		if (query_kallsyms(name)) {
			pr_alert("[nettrace] Note: The function we want to trace (%s) has become "
					"%s bacause the kernel compiler had added a suffix in its symbol! "
					"There may be several versions of %s, please check /proc/kallsyms\n",
					(char *) p->symbol_name, name, (char *) p->symbol_name);
			strncpy((char *) p->symbol_name, name, sizeof(name) - 1);
			if (register_kprobe(p) < 0)
				goto on_err;
		} else {
			goto on_err;
...
int c_register_kretprobe(struct kretprobe *p)
{
	if (register_kretprobe(p) < 0) {
		char name[MAX_TP_NAME] = {};
		strncpy(name, p->kp.symbol_name, sizeof(name) - 1);
		if (query_kallsyms(name)) {
			pr_alert("[nettrace] Note: The function we want to trace (%s) has become "
					"%s bacause the kernel compiler had added a suffix in its symbol! "
					"There may be several versions of %s, please check /proc/kallsyms\n",
					(char *) p->kp.symbol_name, name, (char *) p->kp.symbol_name);
			strncpy((char *) p->kp.symbol_name, name, sizeof(name) - 1);
			if (register_kretprobe(p) < 0)
				goto on_err;
		} else {
			goto on_err;
