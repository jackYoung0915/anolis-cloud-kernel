...
#define REGISTER_KPROBE(func)											\
	...
	{																	\
		printk(KERN_INFO "register_kprobe failed:" #func "\n");			\
		return 0;														\
	}																	\
	printk(KERN_INFO "Planted kprobe at %p, handler addr %p, name %s\n",\
		kprobe_##func.addr, kprobe_##func.post_handler,					\
		kprobe_##func.symbol_name);
