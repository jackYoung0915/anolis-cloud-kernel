...
static int filter_skb(struct sk_buff *skb, struct sock *sk,
		struct trace_point *tp, struct skb_output *skb_output);

static int filter_sock(struct sock *sk, struct trace_point *tp, struct skb_output *output);

/* This is the general skb info print function.
 *
 * The info in 'output' will be printed by 'log_data' log level.
 */
static void general_print(struct skb_output *output, struct trace_point *tp)
{
...
void __post_handler_general(struct kprobe *p, struct pt_regs *regs, unsigned long flags)
{
	...
	struct sock	     *sk = NULL;
	struct skb_output skb_output = {};
	struct trace_point	     *tp = (struct trace_point *) p->symbol_name;
...
int entry_handler_general(struct kretprobe_instance *ri, struct pt_regs *regs)
{
	struct sk_buff  *skb = NULL;
	struct sock	     *sk = NULL;
	struct trace_point	     *tp = NULL;
	struct ret_data		*data = (struct ret_data *) ri->data;
	...
	if (unlikely(!rp))
		return 1;
	tp = (struct trace_point *) rp->kp.symbol_name;
#else
	tp = (struct trace_point *) ri->rph->rp->kp.symbol_name;
#endif
	if (tp->sock_index > 0)
		sk = (struct sock *) kprobe_parm(regs, tp->sock_index);
