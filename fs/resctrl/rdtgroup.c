			      ...
			      struct seq_file *seq, void *v)
{
	...
	rcu_read_lock();
	list_for_each_entry_rcu(dom, &r->ctrl_domains, hdr.list) {
		if (sep)
			seq_putc(seq, ';');
		sw_shareable = 0;
		exclusive = 0;
					   ...
					   struct rdt_mon_domain *d)
{
	...
	sprintf(name, "mon_%s_%02ld", r->name, snc_mode ? d->ci->id : d->hdr.id);
	if (snc_mode)
...
static void _resctrl_offline_mon_domain(struct rdt_resource *r, struct rdt_mon_domain *d)
{
	lockdep_assert_held(&rdtgroup_mutex);
...
{
	mutex_lock(&rdtgroup_mutex);
	_resctrl_offline_mon_domain(r, d);
	mutex_unlock(&rdtgroup_mutex);
}
...
static int _resctrl_online_mon_domain(struct rdt_resource *r, struct rdt_mon_domain *d)
{
	int err;
...
int resctrl_online_mon_domain(struct rdt_resource *r, struct rdt_mon_domain *d)
{
	...
	mutex_lock(&rdtgroup_mutex);
	err = _resctrl_online_mon_domain(r, d);
	mutex_unlock(&rdtgroup_mutex);
