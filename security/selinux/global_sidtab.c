			    ...
			    u32 scontext_len, u32 *out_sid, gfp_t gfp)
{
	 ...
	 * function, which will correctly handle initial SID contexts but
	 * avoid caching invalid contexts in the global SID table.
	 */
	if (!selinux_initialized(state))
		return selinux_ss_context_to_sid(state, scontext, scontext_len,
						 out_sid, gfp);
...
int selinux_audit_rule_match(u32 sid, u32 field, u32 op, void *vrule)
{
	...
	rc = map_global_sid_to_ss(state, sid, &local_sid, GFP_ATOMIC);
	if (rc)
		return -ENOENT;
	return selinux_ss_audit_rule_match(local_sid, field, op, vrule);
}
