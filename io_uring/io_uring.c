				 ...
				 u64 extra1, u64 extra2)
{
	...
	trace_io_uring_complete(ctx, user_data, res, cflags, extra1, extra2);
...
static noinline void io_fill_cqe_req(struct io_kiocb *req, s32 res, u32 cflags)
{
	if (req->flags & REQ_F_CQE32_INIT)
		...
		__io_fill_cqe(req->ctx, req->user_data, res, cflags, 0, 0);
}
...
{
	ctx->cq_extra++;
	return __io_fill_cqe(ctx, user_data, res, cflags, 0, 0);
}
...
static void io_req_task_work_add(struct io_kiocb *req)
{
	 ...
	 * will do the job.
	 */
	sqpoll_mode = req->ctx->flags & IORING_SETUP_SQPOLL;
	submit_on_idle = req->ctx->sq_data && req->ctx->sq_data->thread &&
			 (req->ctx->sq_data->thread != req->task);
	notify = (!sqpoll_mode || submit_on_idle) ? TWA_SIGNAL : TWA_NONE;
	...
	__must_hold(&ctx->uring_lock)
{
	...
	for (i = 0; i < nr; i++) {
		...
		__io_fill_cqe(req->ctx, req->user_data, req->result, req->compl.cflags,
			      (req->flags & REQ_F_CQE32_INIT) ? req->extra1 : 0,
			      (req->flags & REQ_F_CQE32_INIT) ? req->extra2 : 0);
	}
	io_commit_cqring(ctx);
...
static void io_iopoll_req_issued(struct io_kiocb *req)
{
	...
	if (unlikely(in_async)) {
		 ...
		 * in sq thread task context or in io worker task context or in
		 * original context. If current task context is sq thread, we
		 * don't need to check whether should wake up sq thread.
		 */
		if ((ctx->flags & IORING_SETUP_SQPOLL) &&
...
static void io_sqd_update_thread_idle(struct io_sq_data *sqd)
{
	...
	list_for_each_entry(ctx, &sqd->ctx_list, sqd_list) {
		...
		sq_thread_idle = max(sq_thread_idle, tmp_idle);
	}
				...
				struct io_uring_params *p)
{
	...
	if (ctx->flags & IORING_SETUP_SQPOLL) {
		 ...
		 */
		if (p->flags & IORING_SETUP_IDLE_US)
...
static void __io_uring_show_fdinfo(struct io_ring_ctx *ctx, struct seq_file *m)
{
	...
	for (i = 0; i < sq_entries; i++) {
		...
		seq_printf(m, "%5u: opcode:%d, fd:%d, flags:%x, off:%llu, "
